...
class PascalVOC(IMDB):
    ...
    def image_path_from_index(self, index):
        ...
        if index.startswith("COCO"):
            image_file = os.path.join(self.data_path, 'COCO_JPEGImages', index + '.jpg')
        else:
    ...
    def segmentation_path_from_index(self, index):
        ...
        """
        if index.startswith("COCO"):
            seg_class_file = os.path.join(self.data_path, 'COCO_SegmentationClass', index + '.png')
        else:
    ...
    def load_selective_search_roidb(self, gt_roidb):
        ...
        for i in range(raw_data.shape[0]):
            boxes = raw_data[i][:, (1, 0, 3, 2)] - 1  # pascal voc_cfg dataset starts from 1.
    ...
    def write_segmentation_result(self, pred_segmentations):
        self.write_pascal_segmentation_result(pred_segmentations)
    ...
    def _py_evaluate_segmentation(self):
        ...
        result_dir = os.path.join(self.result_path, 'results', 'VOC' + self.year, 'Segmentation')
        sub_file_folder = os.path.join(self.result_path, 'results', 'VOC' + self.year, 'difference')
        ...
        for i, index in enumerate(self.image_set_index):
            ...
            assert seg_gt.shape[1] == seg_pred.shape[1] and seg_gt.shape[0] == seg_pred.shape[0]
            seg_gt = cv2.resize(seg_gt, (seg_pred.shape[1], seg_pred.shape[0]), interpolation=cv2.INTER_NEAREST)

            sub_save_path = os.path.join(sub_file_folder, index + '.png')
            ...
            with open(result_analyzation_file,'a') as f:
                ...
                f.writelines(index +'\t' + str(oneIU*100) + '\t' +'mean:'+str(oneMeanIU)+'\n')
