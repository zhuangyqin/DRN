...
namespace mshadow{
    ...
    inline void SoftmaxOHEMGrad(Tensor<cpu, 3, DType> dst,
        ...
        for (openmp_index_t n = 0; n < dst.size(2); ++n) {
            for (index_t y = 0; y < dst.size(0); ++y) {
                ...
                if (k == static_cast<int>(ignore_label)|| src[y][k][n]>thresh) {
...
namespace mxnet {
    namespace op {
        ...
        template<>
        Operator *CreateOp<cpu>(SoftmaxOHEMOutputParam param, int dtype) {
            Operator *op = NULL;
            MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
                op = new SoftmaxOHEMOutputOp<cpu, DType>(param);
            })
            return op;
        }

// DO_BIND_DISPATCH comes from operator_common.h
        Operator *SoftmaxOHEMOutputProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
                                                          std::vector<int> *in_type) const {
            DO_BIND_DISPATCH(CreateOp, param_, (*in_type)[0]);
        }

        DMLC_REGISTER_PARAMETER(SoftmaxOHEMOutputParam);

        MXNET_REGISTER_OP_PROPERTY(SoftmaxOHEMOutput, SoftmaxOHEMOutputProp)
...
- During backward propagation, the gradient of cross-entropy loss w.r.t softmax output array is computed.
  ...
  - This operator also supports various ways to normalize the gradient by `normalization`,
    ...
    - ``'valid'``: divide the gradient by the number of instances which are not ignored.
                ...
                .add_arguments(SoftmaxOHEMOutputParam::__FIELDS__());
