...
def parse_args():
    parser = argparse.ArgumentParser(description='Test a Deeplab Network without label')
    # general
...
from core.tester import Predictor, pred_eval,Predictor_Emsemble
from utils.load_data import load_gt_segdb, merge_segdb
from utils.load_model import load_param,load_param_emsemble
...
def test_deeplab():
    ...
    if config.TEST.get('USE_EMSEMBLE', False):
        ...
        mctx = ctx
# get test data iter
    test_data = TestDataLoader(segdb, config=config, batch_size=batch_size,shuffle=False,ctx=mctx,has_label=False)
    ...
    if not use_emsemble:
        ...
        arg_params_list, aux_params_list = load_param_emsemble(prefix, epoch, num_emsemble, process=True)
        ...
        label_names = ['label']
        max_data_shape = [[('data', (1, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES])))]]
        predictor = Predictor_Emsemble(sym, data_names, label_names,
                                       context=ctx, max_data_shapes=max_data_shape,
                                       provide_data=test_data.provide_data, provide_label=test_data.provide_label,
                                       arg_params_list=arg_params_list, aux_params_list=aux_params_list)

    # start detection
    pred_eval(predictor, test_data, imdb, vis=args.vis, ignore_cache=args.ignore_cache, logger=logger,has_label=False)
