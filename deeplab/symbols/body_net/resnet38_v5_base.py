...
def BN(data,name=None, suffix=''):
    ...
    bn = mx.symbol.BatchNorm(data=data, name=('bn%s' % suffix), eps=eps,
                             use_global_stats=use_global_stats, fix_gamma=fix_gamma,momentum=momentum)
    return bn

def AC_CONV(data, num_filter, kernel, stride=(1, 1), pad=(0, 0), dilate=(1, 1), name=None, suffix=''):
    act = ReLU(data,name=('%s_reldu' % name))
...
def BN_AC(data,name=None, suffix=''):
    ...
    bn = mx.symbol.BatchNorm(data=data, name=('bn%s' % suffix), eps=eps, use_global_stats=use_global_stats,
                             fix_gamma=fix_gamma,momentum=momentum)
...
def get_conv_feature(data,is_train=True,workspace=2048,fix_gamma=True,use_global_stats=True,eps =1.001e-5,momentum_ = 0.9):
    global momentum
    momentum = momentum_
