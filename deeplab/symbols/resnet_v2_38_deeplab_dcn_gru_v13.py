...
from layers.distloss import make_distloss
from layers.OhemSoftmax import OhemSoftmax, OhemSoftmaxProp
from layers.CenterLoss import CenterLoss, CenterLossProp

# this version is the parrell rnn
class resnet_v2_38_deeplab_dcn_gru_v13(Symbol):
    def __init__(self):
        """
    ...
    def get_fcn_top(self, conv_feature, label, name, num_classes, cfg, is_train,grad_scale=1.0):
        ...
        if cfg.TRAIN.loss_type == "CE" or not is_train:
            softmax = mx.symbol.SoftmaxOutput(data=linear19, label=label, normalization='valid',
                                              multi_output=True,
                                              use_ignore=True, ignore_label=255, name="softmax",grad_scale=grad_scale)
            ...
            softmax = mx.symbol.SoftmaxOHEMOutput(data=linear19, label=label, multi_output=True,name="softmax",
    ...
    def get_centerloss_top(self, conv_feature, label, num_class, alpha= 0.05, grad_scale=1.0, skip_step = 1, scale_name=''):
        center_loss = mx.sym.Custom(conv_feature, label, op_type='centerloss',name='center_loss_' + str(skip_step) + scale_name,
                                    num_class=num_class, alpha=alpha,  grad_scale=grad_scale, clip_grad=5.0)
    ...
    def get_train_symbol(self, num_classes, cfg):
        ...
        metric_grad_scale = float(array[1])
        print "center grad scale", metric_grad_scale
        ...
        for idx, i in enumerate(cfg.network.scale_list):
            ...
            rnn_feature = RnnMap_cudnn_v2(P_feature[3], name="RNNRelation_"+str(i)+scale_name[idx], type=self.rnn_type, PatchSize=self.patch_size,
                           num_hidden=num_hidden, use_memory=True,old_type=False,skip_step=i,parallel=True)
            rnn_feature_list.append(rnn_feature)
            if not cfg.TRAIN.use_one_center:
                center_top_list.append(self.get_centerloss_top(conv_feature=rnn_feature, label=seg_cls_gt,num_class = num_classes, alpha = cfg.TRAIN.alpha,grad_scale=array[1],skip_step=i,scale_name=scale_name[idx]))
        ...
        if cfg.TRAIN.use_one_center:
            center_top = self.get_centerloss_top(conv_feature=rnn_feature, label=seg_cls_gt, num_class=num_classes,
                                alpha=cfg.TRAIN.alpha, grad_scale=array[1], skip_step=i, scale_name=scale_name[idx])
            loss = [fcn_fusion,center_top]
            sym = mx.symbol.Group(loss)

        else:
            loss = [fcn_fusion, center_top_list[0],center_top_list[1],center_top_list[2]]
            sym =  mx.symbol.Group(loss)
        return sym
    ...
    def get_test_symbol(self, num_classes,cfg):
        ...
        for idx, i in enumerate(cfg.network.scale_list):
            ...
            rnn_feature = RnnMap_cudnn_v2(P_feature[3], name="RNNRelation_" + str(i)+scale_name[idx], type=self.rnn_type,
                                          PatchSize=self.patch_size,
                                          num_hidden= num_hidden, use_memory=True, old_type=False,
                                          skip_step=i,parallel=True)
            rnn_feature_list.append(rnn_feature)
            ...
            if k not in arg_params:
                ...
                if k.endswith("linear_weight"):
                    ...
                    if k.endswith('state'):
                        ...
                        print "k", k, self.arg_shape_dict[k]
                        init_base =  mx.init.Xavier(rnd_type='gaussian', factor_type='in', magnitude=1)
                        if cfg.network.use_weight:
                            down_stage = int(k[-10])
                            rnn_init = mx.init.FusedRNN(init_base, num_layers=1,
                                                        num_hidden=self.num_hidden / down_stage, mode="gru",
                                                        bidirectional=True)
                            arg_params[k] = mx.nd.zeros(shape=self.arg_shape_dict[k])
                            rnn_init._init_weight(mx.init.InitDesc(k), arg_params[k])
                        else:
                            rnn_init = mx.init.FusedRNN(init_base, num_layers=1, num_hidden=self.num_hidden, mode="gru",
                                                        bidirectional=True)
                            arg_params[k] = mx.nd.zeros(shape=self.arg_shape_dict[k])
                            rnn_init._init_weight(mx.init.InitDesc(k), arg_params[k])
